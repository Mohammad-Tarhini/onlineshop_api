...
namespace onlineshopowner_api.Infrastructure.Repositories
{
   ...
   string searchbycategory = null)
        ...
        public async Task<string> SetProductInRedis(ProductDto product)
        {
            try
            {
        ...
        public async Task<(string status,List<ProductDto> productDtos,string message)> GetProductFromRedis(int shopid = 0, int limit = 30, int pagenb = 1, string searchbyproductname = null,  string searchbyproductcategory = null,string searchbytype=null)
        {
            try
            {
                ...
                var filteredShops = new List<ProductDto>();
                int countkey = 0;
                // Step 2: Load shops and apply filtering
                foreach (var redisKey in allKeys)
                {
                    ...
                    if (countkey > limit + offset)
                        break;
                    var product = await _rediscache.GetObjectAsync<ProductDto>(redisKey);
